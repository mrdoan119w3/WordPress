<div itemscope itemtype="http://schema.org/WebSite" class="search_form_wrap">
    ...
    <form itemprop="potentialAction" itemscope itemtype="http://schema.org/SearchAction" method="get" class="form-inline" id="searchform" action="<?php echo esc_url( home_url( '/' ) ); ?>">
		...
		<input itemprop="query-input" type="text" class="form-control field" value="<?php echo get_search_query()?>" name="s" id="s" placeholder="<?php esc_attr_e( 'Searching...', 'cptdev' ); ?>" />
		<button type="submit" class="btn btn-default" id="searchsubmit"><?php esc_attr_e( 'Search', 'cptdev' ); ?></button>
    </form>
</div>