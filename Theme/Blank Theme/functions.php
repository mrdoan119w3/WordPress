...
define( "CPTDEV_VERSION" ,'1.0');
define( "CPTDEV_DEV_MODE" ,true);
if( class_exists('acf') ) { 
 ...
 * Setup theme
 */
function cptdev_setup() {
	load_theme_textdomain( 'cptdev', get_template_directory() . '/languages' );
	...
	register_nav_menus( array(
		'header' => __( 'Header menu', 'cptdev' ),
		'footer'  => __( 'Footer menu', 'cptdev' ),
	));
	/*
...
add_action( 'after_setup_theme', 'cptdev_setup' );
//Sidebar
/*
...
function svl_wp_title( $title, $sep ) {
	...
	if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() )
		$title = "$title $sep " . sprintf( __( 'Trang %s', 'cptdev' ), max( $paged, $page ) );
	return $title;
}
add_filter( 'wp_title', 'svl_wp_title', 10, 2 );

// Add specific CSS class by filter
add_filter( 'body_class', 'cptdev_mobile_class' );
function cptdev_mobile_class( $classes ) {
	if(wp_is_mobile()){
		$classes[] = 'cptdev_mobile';
	}else{
		$classes[] ="cptdev_desktop";
	}
	return $classes;
}
/* ACF 4
...
function wp_corenavi_table($main_query = null) {
		...
		echo paginate_links( array(
			...
			'total' 	=> $total,
			'mid_size'	=> '10',
			'prev_text'    => __('Trang trước','cptdev'),
			'next_text'    => __('Trang tiếp','cptdev'),
		) );
		if($total > 1) echo '</div>';
}
...
function get_excerpt($limit = 130){
	...
	if($excerpt){
		...
		$excerpt = $excerpt.'... <a href="'.$permalink.'" title="" rel="nofollow">'.__('Xem thêm','cptdev').'</a>';
	}
	return $excerpt;
}

function cptdev_excerpt_more( $more ) {
    return '...';
}
add_filter( 'excerpt_more', 'cptdev_excerpt_more' );

if ( ! function_exists( 'cptdev_ilc_mce_buttons' ) ) {
    function cptdev_ilc_mce_buttons($buttons){
        array_push($buttons,
            "alignjustify",
            "subscript",
            "superscript"
    ...
    add_filter("mce_buttons", "cptdev_ilc_mce_buttons");
}
if ( ! function_exists( 'cptdev_ilc_mce_buttons_2' ) ) {
    function cptdev_ilc_mce_buttons_2($buttons){
        array_push($buttons,
            "backcolor",
            "anchor",
            "fontselect",
    ...
    add_filter("mce_buttons_2", "cptdev_ilc_mce_buttons_2");
}
// Customize mce editor font sizes
if ( ! function_exists( 'cptdev_mce_text_sizes' ) ) {
    function cptdev_mce_text_sizes( $initArray ){
        $initArray['fontsize_formats'] = "9px 10px 12px 13px 14px 16px 17px 18px 19px 20px 21px 24px 28px 32px 36px";
        return $initArray;
    }
    add_filter( 'tiny_mce_before_init', 'cptdev_mce_text_sizes' );
}

add_action( 'tgmpa_register', 'cptdev_register_required_plugins' );
function cptdev_register_required_plugins() {
	$plugins = array(
		array(
	...
	$config = array(
		'id'           => 'cptdev',                 // Unique ID for hashing notices for multiple instances of TGMPA.
