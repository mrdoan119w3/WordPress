...
if ( is_plugin_active( 'baw-post-views-count/bawpv.php' ) ) {
    class cptdev_BAW_Widget_Most_Viewed_Posts extends WP_Widget
    {

        function __construct()
        {
            $widget_ops = array('classname' => 'cptdev_widget_most_viewed_entries', 'description' => __('The most viewed posts on your site', 'cptdev'));
            parent::__construct('cptdev_widget_most_viewed_entries', __('[cptdev] Most Viewed Posts With Thumbnail', 'cptdev'), $widget_ops);
            $this->alt_option_name = 'cptdev_widget_most_viewed_entries';
        ...
        {
            $cache = wp_cache_get('cptdev_widget_most_viewed_entries', 'widget');
            if (!is_array($cache))
                $cache = array();
            ...
            if ($r->have_posts()) :
                ?>
                <style>
                    .cptdev_widget_most_viewed_entries ul {
                        list-style: none;
                        padding: 0;
                        margin: 0;
                    }
                    .cptdev_widget_most_viewed_entries ul li {
                        overflow: hidden;
                        margin: 0 0 10px 0;
                    }
                    .cptdev_widget_most_viewed_entries ul li img {
                        width: 90px;
                        height: auto;
                        float: left;
                        margin: 0 10px 0 0;
                    }
                    .cptdev_widget_most_viewed_entries ul li strong {
                        margin: 0 0 10px;
                        display: block;
                    }
                </style>
            ...
            wp_cache_set('cptdev_widget_most_viewed_entries', $cache, 'widget');
        }
        ...
        function update($new_instance, $old_instance)
        {
            ...
            if (isset($alloptions['cptdev_widget_most_viewed_entries']))
                delete_option('cptdev_widget_most_viewed_entries');
        ...
        {
            wp_cache_delete('cptdev_widget_most_viewed_entries', 'widget');
        }
        ...
        function form($instance)
        {
            ...
            if (isset($instance['order']))
                ...
                $order = 'DESC';
            ?>
            <p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'cptdev'); ?></label>
                <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>"
                       name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>"/>
            </p>

            <p>
                <label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('How many posts:', 'cptdev'); ?></label>
                <input id="<?php echo $this->get_field_id('number'); ?>"
                       name="<?php echo $this->get_field_name('number'); ?>" type="text" value="<?php echo $number; ?>"
                       size="3"/></p>

            <p>
                <label for="<?php echo $this->get_field_id('post_type'); ?>"><?php _e('Post type:', 'cptdev'); ?></label>
            ...
            <p>
                <label for="<?php echo $this->get_field_id('time'); ?>"><?php _e('Which top do you want:', 'cptdev'); ?></label>
            ...
            <p>
                <label for="<?php echo $this->get_field_id('author'); ?>"><?php _e('Top for this author only:', 'cptdev'); ?></label>
                <select id="<?php echo $this->get_field_id('author'); ?>"
                        name="<?php echo $this->get_field_name('author'); ?>">
                    <option value=""><?php _e('All authors', 'cptdev'); ?></option>
                ...
                <?php /* // todo
		<p><label for="<?php echo $this->get_field_id('author'); ?>"><?php _e( 'Exclude categories: (Multiple choise possible)', 'cptdev' ); ?></label>
		...
		*/ ?>
            <p><label for="<?php echo $this->get_field_id('date'); ?>"><?php _e('Date format', 'cptdev'); ?> <code>YYYYMMAA</code></label>
                ...
                <code><?php _e('If you leave blank the actual time will be used.', 'cptdev'); ?></code></p>

            <p>
                <label for="<?php echo $this->get_field_id('show'); ?>"><?php _e('Show posts count:', 'cptdev'); ?></label>
                <input id="<?php echo $this->get_field_id('show'); ?>"
                       name="<?php echo $this->get_field_name('show'); ?>"
                       type="checkbox" <?php checked($show == true, true); ?> /></p>

            <p><label for="<?php echo $this->get_field_id('order'); ?>"><?php _e('Order', 'cptdev'); ?></label>
                <select id="<?php echo $this->get_field_id('order'); ?>"
                        name="<?php echo $this->get_field_name('order'); ?>">
                    <option value="DESC" <?php selected($order, 'DESC'); ?>><?php _e('From most viewed to less viewed', 'cptdev'); ?></option>
                    <option value="ASC" <?php selected($order, 'ASC'); ?>><?php _e('From less viewed to most viewed', 'cptdev'); ?></option>
                </select>
            </p>
    ...
    add_action('widgets_init', 'cptdev_bawpvc_widgets_init');
    function cptdev_bawpvc_widgets_init()
    {
        register_widget('cptdev_BAW_Widget_Most_Viewed_Posts');
    }
}